...
from MDN.metrics import NUM_INT_STEPS
...
def safe_inverse_softplus(x):
    # return torch.log(1 - torch.exp(-x) + 1e-8) + x
    return torch.log(torch.exp(x) - 1 + 1e-6)

def inverse_softplus_grad(x):
    # return 1 / (1 - torch.exp(-x) + 1e-8)
...
class SurvivalMDN_Dist():
    ...
    def pred_time(self):
        ws, mus, sigmas = self.pred_params_to_parameters(self.pred_params)
        normal_dists = torch.distributions.Normal(mus, sigmas)
        yi_sampling = normal_dists.sample(torch.Size([1])).squeeze(dim=0)  # (batch_size, num_components)
        ys = ws * yi_sampling  # (batch_size, num_components)
        ys = ys.sum(-1)  # batch_size
        return torch.log(1 + torch.exp(ys))
...
class MDNModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs["expected_time"] = dist.pred_time()
        outputs["Lambda"] = -torch.log(survival_func + 1e-6)    # a notion of hazard
        outputs["lambda"] = pdf / survival_func                 # another notion of hazard
