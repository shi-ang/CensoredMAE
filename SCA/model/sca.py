...
from SCA.model.risk_network import pz_given_x, pt_given_z
from SCA.utils.accuracy import accuracy_loss
from SCA.utils.metrics import plot_cost, l2_loss, l1_loss
from SCA.utils.clustering import cluster_assignment, update_pop_pi, run_k_means
from SCA.utils.calibration import km_estimator
import math
from SCA.utils.tf_helpers import create_centroids, create_pop_pi, show_all_variables
...
class SCA(object):
    ...
    def _build_graph(self):
        ...
        with self.G.as_default():
            ...
            self.save_path = "SCA/summaries/{0}_model".format(self.model)
    ...
    def train_test(self, train=True):
        with self.session.as_default():
            ...
            if train:
                ...
                np.save("SCA/matrix/Test_centroids", centroids)
                np.save("SCA/matrix/Test_pop_nk", pop_nk)
                np.save("SCA/matrix/learned_K", self.train_k)
    ...
    def save_generated_h2(self, feed_dict, name):
        ...
        np.save("SCA/matrix/{}_z".format(name), z)
    ...
    def generate_statistics(self, data_x, data_e, data_t, name, data_dict, save=True):
        ...
        if name == 'Test':
            ...
            np.save('SCA/matrix/{}_predicted_t_median'.format(name), median_predicted_time)
            np.save('SCA/matrix/{}_empirical_t'.format(name), data_t)
            np.save('SCA/matrix/{}_empirical_e'.format(name), data_e)
            np.transpose(np.save('SCA/matrix/{}_predicted_t_samples'.format(name), predicted_samples))
